...
### Przygotowuję zestaw danych dla API ###
# Definiuję zbiór danych dat transakcyjnych
date_beginning = pendulum.date(2025, 1, 1)
date_end = pendulum.date(2025, 12, 31)

# Zapisuję daty do listy
...
# Generator przykładowych danych transakcyjnych
...
def get_transactions(
    ...
    # Parsowanie daty ze parametru (string) do formatu daty
    try:
        start_date_parsed = pendulum.parse(start_date) if start_date else date_beginning
    except pendulum.parsing.exceptions.ParserError:
        raise HTTPException(status_code=400, detail="start_date ma niepoprawny format, użyj YYYY-MM-DD")

    try:
        end_date_parsed = pendulum.parse(end_date) if end_date else date_end
    except pendulum.parsing.exceptions.ParserError:
        raise HTTPException(status_code=400, detail="end_date ma niepoprawny format, użyj YYYY-MM-DD")
    ...
    # Zwracanie zafiltrowanego zbioru transakcji
